        ...
        <div className="p-6">
          ...
          <div className="mt-8 p-6 bg-gray-50 rounded-xl">
            <div className="flex items-center justify-between mb-4">
              <span className="text-lg font-bold text-gray-800">RÉCAPITULATIF</span>
            </div>
            
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Frais de mise en oeuvre (versement unique)</span>
                <span className="font-bold text-gray-800">{getOneTimePrice()}€ HT</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Abonnement mensuel</span>
                <span className="font-bold text-[#FF0671]">{getMonthlyPrice()}€ HT/mois</span>
              </div>
            ...
            {offeredItems.size > 0 && (
              <div className="mt-6 p-4 bg-[#FF0671]/10 rounded-lg border border-[#FF0671]/20">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-bold text-[#FF0671]">Économies réalisées</span>
                </div>
                <ul className="space-y-1 text-sm text-gray-700">
                  {offeredItems.has("site-vitrine") && (
                    <li>• Site vitrine : <span className="font-bold text-[#FF0671]">
                      {pricingItems.find(i => i.id === "site-vitrine")?.price.toLocaleString('fr-FR')}€ offerts
                    </span></li>
                  )}
                  {offeredItems.has("nom-domaine") && (
                    <li>• Nom de domaine : <span className="font-bold text-[#FF0671]">
                      {pricingItems.find(i => i.id === "nom-domaine")?.price}€ offerts
                    </span></li>
                  )}
                  {offeredItems.has("communication") && (
                    <li>• Communication Web : <span className="font-bold text-[#FF0671]">
                      {pricingItems.find(i => i.id === "communication")?.price}€ offerts
                    </span></li>
                  )}
                  {offeredItems.has("formation") && (
                    <li>• Formation : <span className="font-bold text-[#FF0671]">
                      {pricingItems.find(i => i.id === "formation")?.price}€ offerts
                    </span></li>
                  )}
                  {offeredItems.has("frais-mise-en-oeuvre") && (
                    <li>• Frais de mise en oeuvre : <span className="font-bold text-[#FF0671]">
                      {pricingItems.find(i => i.id === "frais-mise-en-oeuvre")?.price}€ offerts
                    </span></li>
                  )}
                </ul>
                <div className="mt-3 pt-3 border-t border-[#FF0671]/20">
                  <span className="text-lg font-bold text-[#FF0671]">
                    Total économisé : {totalSavings.toLocaleString('fr-FR')}€
                  </span>
                </div>
              </div>
            )}
